    ...
    <div className="flex flex-col h-[calc(100vh-theme(spacing.16))] lg:h-screen w-full bg-white relative">
      <div className="shrink-0 px-6 py-3 border-b border-gray-50 flex items-center justify-between bg-white/90 backdrop-blur-md sticky top-0 z-20">
          <div className="flex items-center gap-4">
              <Link href="/messages" className="lg:hidden p-2 -ml-3 text-gray-400 hover:text-black transition-colors"><ArrowLeft size={22} /></Link>
              <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gray-50 relative overflow-hidden border border-gray-100">
                      {targetUser?.avatar_url ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-sm font-bold text-gray-300">{targetUser?.nickname?.[0]?.toUpperCase()}</div>
                      )}
                  </div>
                  <div className="flex flex-col">
                      <div className="flex items-center gap-1.5">
                        <span className="text-sm font-bold text-gray-900 leading-none">{targetUser?.nickname || "Carregando..."}</span>
                        <VerificationBadge badge={targetUser?.verification_badge} size="sm" />
                      </div>
                      <span className="text-[10px] font-medium text-gray-400 mt-0.5">Online agora</span>
                  </div>
              </div>
          </div>
          <button className="text-gray-300 hover:text-black transition-colors p-2"><MoreVertical size={20} /></button>
      </div>
              ...
              return (
                  <div key={msg.id} className={cn("flex w-full animate-in fade-in slide-in-from-bottom-2 duration-300", isMe ? "justify-end" : "justify-start")}>
                      <div className={cn("flex max-w-[85%] sm:max-w-[70%] gap-2", isMe ? "flex-row-reverse" : "flex-row")}>
                          ...
                          <div className={cn("px-4 py-2.5 text-[15px] leading-relaxed relative transition-all", isMe ? "bg-gradient-to-br from-[#42047e] to-[#6d28d9] text-white rounded-2xl rounded-br-none shadow-sm" : "bg-gray-50 text-gray-800 border border-gray-100/50 rounded-2xl rounded-bl-none", !isFirstInGroup && isMe && "mt-[-10px] rounded-tr-md", !isFirstInGroup && !isMe && "mt-[-10px] rounded-tl-md")}>
                              {msg.content}
                              <div className={cn("text-[9px] font-medium mt-1 flex items-center gap-1 justify-end opacity-80", isMe ? "text-purple-100" : "text-gray-400")}>
                                  {new Date(msg.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                                  {isMe && (msg.read_at ? <CheckCheck size={11} className="text-[#07f49e]" /> : <Check size={11} />)}
                              </div>
                          </div>
                      </div>
                  </div>
      ...
      <div className="shrink-0 p-4 bg-white border-t border-gray-50">
          <form onSubmit={handleSend} className="flex items-center gap-3 bg-gray-50/50 p-1.5 pl-5 rounded-full border border-gray-100 focus-within:border-[#42047e]/20 focus-within:bg-white focus-within:shadow-lg focus-within:shadow-[#42047e]/5 transition-all duration-300">
              <input ref={inputRef} className="flex-1 bg-transparent border-none focus:ring-0 text-[15px] placeholder-gray-400 text-gray-900 h-10" placeholder="Escreva uma mensagem..." value={newMessage} onChange={e => setNewMessage(e.target.value)} disabled={isSending} />
              <button type="submit" disabled={!newMessage.trim() || isSending} className="w-10 h-10 bg-black text-white rounded-full flex items-center justify-center hover:bg-[#42047e] hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:scale-100 disabled:bg-gray-200 shadow-md">
